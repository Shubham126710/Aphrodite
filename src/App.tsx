...
function App() {
  ...
  const [currentPage, setCurrentPage] = useState<'home' | 'auth' | 'manifesto'>('home');
  ...
  return (
    <>
      {isLoading ? (
        ...
        <main className="bg-off-white min-h-screen text-neutral-dark font-sans overflow-x-hidden selection:bg-primary-pink selection:text-off-white perspective-container">
          <div className="noise-overlay fixed inset-0 pointer-events-none z-50"></div>
          
          {currentPage !== 'auth' && <Navbar setCurrentPage={setCurrentPage} />}
          ...
          {currentPage === 'home' && (
            <>
              ...
              <section id="discover" className="relative py-24 md:py-40 px-6 md:px-12 bg-off-white overflow-hidden animate-on-scroll subtle-3d">
                 
                 {/* Subtle Parallax Background Shapes */}
                 <div 
                   className="absolute top-0 right-0 w-96 h-96 bg-soft-blush rounded-full mix-blend-multiply filter blur-3xl opacity-70 z-0 pointer-events-none parallax-bg"
                   data-speed="0.05"
                 ></div>
                 <div 
                   className="absolute bottom-0 left-[-10%] w-[30rem] h-[30rem] bg-light-rose rounded-full mix-blend-multiply filter blur-3xl opacity-50 z-0 pointer-events-none parallax-bg"
                   data-speed="-0.05"
                 ></div>

                 <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-16 items-center relative z-10">
                   
                   <div className="md:w-1/2 stagger-item">
                     <h2 className="font-serif text-5xl md:text-7xl text-deep-rose mb-8">
                       Beyond <span className="italic text-primary-pink">the screen.</span>
                     </h2>
                     <p className="text-neutral-dark/80 text-lg leading-relaxed mb-6 font-medium">
                     ...
                     <p className="text-neutral-dark/80 text-lg leading-relaxed mb-8 font-medium">
                     ...
                     <button onClick={() => setCurrentPage('manifesto')} className="inline-flex items-center text-deep-rose font-bold uppercase tracking-widest text-sm hover:text-primary-pink transition-colors group">
                       Read our manifesto
                       <span className="ml-2 transform group-hover:translate-x-2 transition-transform">→</span>
                     </button>
                   </div>

                   <div className="md:w-1/2 relative stagger-item delay-1">
                     <div className="card-frame p-4 bg-soft-blush rotate-2 hover:rotate-0 transition-transform duration-500 shadow-xl parallax-bg" data-speed="0.02">
                        <img 
                          ...
                          className="w-full h-[500px] object-cover grayscale-[20%] sepia-[20%] contrast-125 rounded filter brightness-110"
                        />
                     </div>
                     {/* Decorative Badge */}
                     <div className="absolute -bottom-8 -left-8 w-24 h-24 bg-primary-pink text-off-white rounded-full flex flex-col items-center justify-center rotate-[-15deg] shadow-lg pointer-events-none parallax-bg" data-speed="0.08">
                       <span className="font-serif italic text-2xl">3</span>
                       <span className="text-[8px] uppercase tracking-widest font-bold">Matches/Wk</span>
                     </div>
                   </div>
              ...
              {/* Features Section */}
              <section id="features" className="py-24 bg-deep-rose text-off-white relative border-y border-neutral-dark/20 text-center animate-on-scroll subtle-3d">
                 <div className="max-w-5xl mx-auto px-6">
                    <span className="font-sans text-xs uppercase tracking-[0.4em] text-light-rose mb-6 block font-bold stagger-item">The Aphrodite Experience</span>
                    <h2 className="font-serif text-4xl md:text-6xl mb-16 italic stagger-item delay-1">Curated for the hopelessly romantic.</h2>
              ...
              <section id="flames" className="py-24 md:py-32 bg-soft-blush border-b border-deep-rose/20 overflow-hidden animate-on-scroll subtle-3d">
                ...
                <div className="max-w-6xl mx-auto px-6 relative z-10">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-6 gap-8 text-center px-4">
                    {[
                      ...
                      <div key={idx} className={`flex flex-col items-center group cursor-default stagger-item delay-${(idx % 6) + 1}`}>
                        <div className="text-7xl md:text-8xl lg:text-8xl font-serif text-deep-rose mb-6 hover:text-primary-pink hover:-translate-y-4 hover:scale-110 transition-all duration-300">
                          {item.letter}
                        </div>
                        <div className="h-[1px] w-8 bg-primary-pink/50 group-hover:w-16 group-hover:bg-deep-rose transition-all duration-500 mb-6"></div>
                        <div className="h-32 overflow-hidden relative w-full pt-2">
                           <h3 className="font-serif text-xl lg:text-2xl text-deep-rose mb-2 font-medium">{item.word}</h3>
                           <p className="text-neutral-dark/80 text-xs lg:text-xs font-sans italic opacity-0 transform translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 delay-100">{item.desc}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {/* About Us Section */}
              <section id="about-us" className="py-24 md:py-32 bg-off-white overflow-hidden animate-on-scroll subtle-3d">
                <div className="max-w-5xl mx-auto px-6 relative z-10">
                   <div className="text-center mb-20 stagger-item">
                      <span className="font-sans text-xs uppercase tracking-[0.4em] text-deep-rose font-bold block mb-4">The Creators</span>
                      <h2 className="font-serif text-5xl md:text-7xl text-deep-rose italic">Architects of Romance</h2>
                   </div>
                   
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-16 lg:gap-24 relative">
                      {/* Decorative line connecting people */}
                      <div className="hidden md:block absolute top-[40%] left-1/4 right-1/4 h-[0.5px] border-t border-dashed border-deep-rose/30"></div>

                      {/* Person 1 */}
                      <div className="flex flex-col items-center text-center group stagger-item delay-1">
                        <div className="w-48 h-48 md:w-56 md:h-56 rounded-t-full rounded-b-[4px] bg-light-rose mb-8 relative overflow-hidden border-[0.5px] border-deep-rose/30 shadow-[4px_4px_0_0_rgba(158,58,68,0.15)] flex items-center justify-center transition-colors duration-500">
                        ...
                        <h3 className="font-serif text-3xl font-medium text-deep-rose mb-2">Shubham Upadhyay</h3>
                        <p className="font-sans text-[10px] uppercase tracking-widest text-primary-pink font-bold mb-4">Co-Founder</p>
                        <p className="text-neutral-dark/80 text-sm leading-relaxed max-w-sm">Detailing the vision of modern romance. [More details perfectly poised to be added here soon, connecting logic to poetry.]</p>
                      </div>
